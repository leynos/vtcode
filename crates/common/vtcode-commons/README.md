...
This crate provides the foundational building blocks that both the core agent
library (`vtcode-core`) and the terminal UI (`vtcode-ui`) depend on. Modules
include ANSI processing, diff rendering, file traversal, color policy, error
classification, and shared protocol types.
...
- `PathResolver` / `PathScope` / `WorkspacePaths` – workspace-aware path
  resolution
