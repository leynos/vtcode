...
Passive JSON schemas for VT Code utility, file, scheduling, and collaboration
tool surfaces.

This crate provides ready-made `serde_json::Value` parameter schemas for the
built-in tool surfaces (apply-patch, cron, exec, search, and collaboration/HITL
tools) so that callers never have to hand-roll JSON Schema objects.

<!-- cargo-rdme start -->

Passive JSON schemas for utility, file, scheduling, and collaboration tool
surfaces.
...
See
[docs.rs/vtcode-utility-tool-specs](https://docs.rs/vtcode-utility-tool-specs)
...
- `DEFAULT_APPLY_PATCH_INPUT_DESCRIPTION` – default input description for
  apply-patch
...
- `cron_create_parameters()`, `cron_list_parameters()`,
