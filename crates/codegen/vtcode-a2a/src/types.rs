...
pub struct Task {
    ...
    /// Artefacts produced by the agent
    #[serde(skip_serializing_if = "Vec::is_empty", default)]
    /// Wire key `artifacts` is fixed by the A2A schema.
...
mod tests {
    ...
    fn test_artefact_creation() {
        ...
        assert_eq!(artefact.id, "art-1");
        assert_eq!(artefact.name, Some("output.txt".to_string()));
        assert_eq!(artefact.parts.len(), 1);
    }
}
