...
mod tests {
    ...
    fn test_detect_email() {
        let tokenizer = PiiTokenizer::new().expect("default PII patterns should compile");
        let text = "Contact me at john@example.com for more info";
        let detected = tokenizer.detect_pii(text).expect("email PII detection should succeed");
    ...
    fn test_detect_phone() {
        ...
        let detected = tokenizer.detect_pii(text).expect("phone-number PII detection should succeed");
    ...
    fn test_tokenize_string() {
        ...
        let (tokenized, tokens) = tokenizer.tokenize_string(text).expect("PII tokenization should succeed");
    ...
    fn test_no_pii_detected() {
        ...
        let detected = tokenizer.detect_pii(text).expect("PII detection should succeed for plain text");
