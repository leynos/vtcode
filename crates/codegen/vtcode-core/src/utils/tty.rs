...
impl TtyExt for io::Stdout {
    ...
    fn is_interactive(&self) -> bool {
        self.is_terminal() && self.supports_colour()
    }
}
...
mod tests {
    ...
    fn capability_predicates_require_their_declared_features() {
        ...
        assert!(!no_colour.is_fully_featured(), "missing colour support should disable fully featured mode",);
        assert!(!no_colour.is_basic_tui(), "missing colour support should disable basic TUI mode",);
