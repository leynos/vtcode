...
- Support the existing slash-style review inputs such as `--last-diff`,
  `--target <expr>`, `--file <path>`, positional file paths, and
  `--style <style>`.
- If the input is empty, review the current diff.
- Focus on bugs, regressions, correctness risks, and missing tests before any
  summary.
