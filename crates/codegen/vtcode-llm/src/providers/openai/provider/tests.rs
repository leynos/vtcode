    ...
    Mock::given(method("POST")).and(path("/responses"))
        .respond_with(|req: &wiremock::Request| {
            ...
            ResponseTemplate::new(400)
                ...
                .set_body_string(r#"{"error":{"message":"customer-record=confidential-123","type":"invalid_request_error","param":"text.verbosity","code":"unsupported_parameter"}}"#)
...
async fn responses_stream_error_keeps_header_metadata_without_provider_body() {
    ...
    Mock::given(method("POST"))
        ...
        .respond_with(
            ResponseTemplate::new(400)
                .insert_header("x-request-id", "req_stream_123")
                .insert_header("retry-after", "15")
                .insert_header("x-ratelimit-limit-requests", "60")
    ...
    {
        Err(error) => error,
        Ok(_) => panic!("stream should surface the HTTP error"),
    };

    assert_header_metadata_without_provider_body(&error, "req_stream_123");
}
...
async fn responses_normalized_stream_error_keeps_header_metadata_without_provider_body() {
    ...
    Mock::given(method("POST"))
        ...
        .respond_with(
            ResponseTemplate::new(400)
                .insert_header("x-request-id", "req_normalized_123")
                .insert_header("retry-after", "15")
                .insert_header("x-ratelimit-limit-requests", "60")
                .set_body_string(r#"{"error":{"message":"customer-record=confidential-123"}}"#),
        )
    ...
    {
        Err(error) => error,
        Ok(_) => panic!("normalized stream should surface the HTTP error"),
    };

    assert_header_metadata_without_provider_body(&error, "req_normalized_123");
}
