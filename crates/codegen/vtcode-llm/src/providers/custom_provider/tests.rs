...
#[tokio::test]
async fn openai_chat_streaming_errors_preserve_configured_rate_limit_headers() {
    assert_custom_rate_limit(openai_chat_rate_limit_error(OpenAiChatTransport::Streaming).await);
}

#[tokio::test]
async fn openai_chat_buffered_errors_preserve_configured_rate_limit_headers() {
    assert_custom_rate_limit(openai_chat_rate_limit_error(OpenAiChatTransport::Buffered).await);
}
