...
mod tests {
    ...
    #[test]
    fn ui_colour_aliases_serialize_to_canonical_wire_keys() {
        let british: UiConfig = toml::from_str("safe_colours_only = true\ncolour_scheme_mode = \"dark\"")
            .expect("British UI colour aliases should deserialize");

        assert!(british.safe_colours_only, "the British safe-colours alias should enable the setting");
        assert_eq!(
            british.colour_scheme_mode,
            ColourSchemeMode::Dark,
            "the British colour-scheme alias should select dark mode"
        );

        let serialized = toml::to_string(&british).expect("the UI configuration should serialize");
        assert!(
            serialized.contains("safe_colors_only = true"),
            "serialization should retain the canonical safe-colours wire key"
        );
        assert!(
            serialized.contains("color_scheme_mode = \"dark\""),
            "serialization should retain the canonical colour-scheme wire key"
        );
        assert!(
            !serialized.contains("safe_colours_only"),
            "serialization should not emit the British safe-colours alias"
        );
        assert!(
            !serialized.contains("colour_scheme_mode"),
            "serialization should not emit the British colour-scheme alias"
        );
