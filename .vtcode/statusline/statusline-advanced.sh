...
# Supports ANSI colour codes for enhanced visual display
...
# Format reasoning effort if present with colour coding
REASONING_DISPLAY=""
...
# Format context information if available with colour coding based on utilization
CONTEXT_DISPLAY=""
if [ "$CONTEXT_UTIL" != "0" ] && [ "$CONTEXT_UTIL" != "null" ]; then
    # Colour code based on context utilization
    if (( $(echo "$CONTEXT_UTIL > 80" | bc -l) )); then
        CONTEXT_COLOUR="\033[31m"  # Red for high utilization
    elif (( $(echo "$CONTEXT_UTIL > 50" | bc -l) )); then
        CONTEXT_COLOUR="\033[33m"  # Yellow for medium utilization
    else
        CONTEXT_COLOUR="\033[32m"  # Green for low utilization
    fi
    CONTEXT_DISPLAY=" | ctx: ${CONTEXT_COLOUR}${CONTEXT_UTIL}%\033[0m (\033[36m${CONTEXT_TOKENS}\033[0m tokens)"
fi

# Build the status line with colours
