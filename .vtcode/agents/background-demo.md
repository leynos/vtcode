...
Use `unified_exec` exactly once to start
`./scripts/demo-background-subagent.sh` from the workspace root as a detached
background process. Do not wait for heartbeat output. Do not call
`unified_exec` more than once. After the tool returns, reply with exactly one
readiness line that includes the launched PID or a concise confirmation that
the process started and ends with `task complete.`, then stop.
