...
---
# User
Refactor `src/parser.rs` to use the new `TokenStream` API. The function on line
142 needs to call `tokens.into_iter().collect()` instead of `tokens.collect()`.

# Assistant
I will locate and read the function first, then apply a targeted patch.

1. Call `code_search` with `query` set to `parse_tokens`, `path` set to
   `src/parser.rs`, and `result_types` set to `["definition"]`.
2. Call `exec_command` with a targeted `sed` command that covers the function
   and nearby context.
