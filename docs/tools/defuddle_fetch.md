...
Fetch cleaned markdown for a URL by calling `web_fetch` with
`format="markdown"`:
...
`web_fetch` (default `format="summary"`) returns a payload that is hard to
parse (heavy JS, paywalled HTML, raw RSS, etc.).
...
- **Session cap**: hard-capped at **one call per session** (the service is
  rate-limited)
- **Max bytes**: 256 KB response limit
- **Timeout**: 30s default, max 60s
- Returns a structured error when the cap is hit, directing the agent back to
  `web_fetch`

## Related Tools

- `web_fetch` — fetch full page content (unlimited calls, more robust);
  `format="markdown"` gives defuddle extraction
