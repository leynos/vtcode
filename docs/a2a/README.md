...
-   **Task**: Represents a stateful unit of work with ID, status, artefacts, and history
...
-   **TaskState**: Lifecycle states - submitted, working, completed, failed, cancelled (`canceled` on the wire), etc.
-   **Artefact**: Tangible output generated by tasks

### Task Manager (`task_manager.rs`)

-   In-memory task storage with concurrent access via `Arc<RwLock>`
-   Task creation, retrieval, status updates, artefact management
...
// Add an artefact
let artefact = Artefact::text("output", "Generated content");
manager.add_artefact(&task.id, artefact).await?;
```
...
-   `-32002` (TaskNotCancellable): Cannot cancel task in current state
