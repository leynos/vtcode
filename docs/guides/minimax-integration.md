...
You can mirror Droid's custom model config using VT Code's user config
(`vtcode.toml` in the canonical platform config directory). Add a `minimax`
provider entry with base URL and API key:
