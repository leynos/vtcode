...
[Bloaty](https://github.com/google/bloaty) is a size profiler for binaries that
helps understand what's making the binary large. This report analyzes the
vtcode project binary size composition.
...
**Size Reduction:** Release-fast profile reduces binary size by **~62%** (52
MiB saved)
...
**Key Insight:** 94% of the binary is code (`__TEXT` segment), which is typical
for Rust binaries.
...
This comes from the `tree-sitter-swift` feature and other tree-sitter language
parsers.
...
- Consider loading tree-sitter grammars dynamically instead of embedding all
  parse tables
...
- Enable more aggressive LTO: `lto = "fat"` (currently `lto = "thin"` in
  release-fast)
- Use `opt-level = "z"` or `opt-level = "s"` for size optimization in
  non-critical paths
- Review large constant data structures (like `base_function_declarations` at
  192 KiB)

### 4. **Debug Info Management**

- The `bloaty` profile has been added to Cargo.toml for analysis builds
- Consider using `split-debuginfo` for production to reduce binary size while
  keeping debug info separate

### 5. **Dependency Audit**

- Run `cargo bloat` (different tool) to identify which dependencies contribute
  most to binary size
...
The vtcode binary is **32 MiB** in release-fast mode, which is reasonable for a
Rust application with:
...
**Potential optimizations** could reduce the binary by 10-20% without
significant functionality loss, primarily through:
