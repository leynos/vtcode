...
This document explains how vtcode implements the loop engineering pattern
described by [Addy Osmani](https://addyosmani.com/blog/loop-engineering/), and
maps each primitive to the module that implements it.
...
The verify closure runs synchronously inside `spawn_blocking` to avoid `Send`
constraint issues with the recursive spawn chain. The full verifier sub-agent
path (`verify_proposed_change`) is available for non-reconciliation
verification flows.
...
- `sqlite` — enables `SqliteLoopMemory` backed by `rusqlite` (bundled SQLite).
  Without this flag, only `MarkdownLoopMemory` is available.
