...
This document explains the cross-compilation setup for the VT Code project
using the `cross` tool.

## Overview

VT Code uses [cross](https://github.com/cross-rs/cross) for reproducible
cross-compilation builds across multiple platforms. The configuration is
defined in `Cross.toml` to ensure consistent builds for all target platforms.
...
- `PKG_CONFIG_ALLOW_CROSS=1` - Allows pkg-config to work during
    cross-compilation
...
1. **Docker Permission Denied**: Ensure Docker is running and your user has
   Docker permissions
2. **Missing SDKROOT**: On macOS, ensure Xcode command line tools are installed
3. **Slow Builds**: Cross-compilation can be slower than native builds due to
   emulation
