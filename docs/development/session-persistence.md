...
dedicated blocking drain. Filesystem I/O does not run on the async executor. If
a bounded queue fills, persistence fails closed and the run cannot be reported
as successful; accepted events are drained before the failure is returned. A
run may be reported as successful only after its terminal `thread.completed`
event and the canonical drain have completed successfully.
...
write failures without changing the canonical event contract. No global the
user state directory's `sessions` harness file is created by default. ATIF and
Open Responses exports configured by the interactive harness are derived under
`<session>/derived/`.
