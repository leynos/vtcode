...
Provides a unified interface for multiple LLM providers with streaming, tool
calling, and provider-specific adaptations. This is a partial extraction from
`vtcode-core`; integration-point files remain there.
...
- `ProviderConfig` is defined here AND in vtcode-core (identical fields) — will
  merge when CGP integration is decoupled
- `system_prompt.rs` provides stub getters with `OnceLock` setters; vtcode-core
  overrides at init
- Provider trait uses feature gates: `#[cfg(feature = "copilot")]`,
