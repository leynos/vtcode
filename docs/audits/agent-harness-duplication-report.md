...
**Date:** 2026-06-05 (initial audit) / 2026-06-06 (implementation) **Scope:**
`crates/codegen/vtcode-core/src/core/agent/**`,
`crates/codegen/vtcode-core/src/prompts/**`,
`crates/codegen/vtcode-core/src/tools/**`, `src/agent/runloop/unified/**`
...
system-prompt assembly — instead of sharing a single path. This is the source
of most duplication below.
...
> `truncate_description`, `truncate_chars`, and `wrap_text_words` (2 local
> copies
>
...
Char-based head/tail/middle truncation was duplicated across the tree. The
table below shows the original sites and their current status:
...
> 1 is `&str` slice (signature), 1 has specialized sanitization logic. The
> shared
...
- The actual per-call rate limiter is a **single `SafetyGateway`** owned by the
  ...
  - Runner → `execute_prepared_public_tool_request` (`tool_access.rs:166`)
    passes
...
- Runner: client-side `LoopDetector` (`runner.rs:103`) checked *before*
  execution in `check_for_loop`; hard limit → `TaskOutcome::LoopDetected` ends
  the turn.
...
- **The two *live* builders already share the canonical base.**
  - Runner: `compose_task_system_prompt` returns the prebuilt
    `self.system_prompt`
  ...
  - Planning workflow and contract text is centralized in `prompts/system.rs`.
    No duplicated section text.
...
- **`SystemPromptGenerator` (`prompts/generator.rs`) was dead code.** Its entire
  ...
  re-export at `prompts/mod.rs:25` — zero call sites workspace-wide. The live
  ACP path uses the *async*
  `prompts::system::generate_system_instruction_with_config` instead
  (`crates/codegen/vtcode-acp/src/zed/session.rs:54`).
...
- `reduce_tool_result` (`harness_kernel.rs:399`, used only by the runner) does
  **semantic content reduction**: dedupes grep matches, caps file/command
  output lines, normalizes search results.
- `compact_model_tool_payload` (`response_content.rs:27`, used only by the
  ...
  (`wall_time`, `spool_hint`, `success`, `status`, …) before the payload
  reaches the model.
...
consolidated 7 of 12 truncation/word-wrap sites into
`vtcode-commons::formatting` (the remaining 5 have different signatures or
semantics). F4 deleted 260 lines of dead code (`prompts/generator.rs`). F3 and
F5 did not survive line-by-line reading. F2's rate-limiting claim was wrong (a
single shared `SafetyGateway` already governs both loops); only its
loop-detection mechanism genuinely differs. The headline: the agent/harness
layer is far less duplicated than a surface scan suggests — assembly is already
centralized in `ToolRegistry`/`SafetyGateway` and `prompts/system.rs`.
...
1. **F2 (loop detection only)** — the runner uses a client-side `LoopDetector`
   ...
   match? Reconciling them changes turn-termination behavior. (Rate-limiting
   and policy admission are already shared via a single `SafetyGateway` — no
   change needed there.)
2. **F5/F2 family** — each loop economizes model context differently
   ...
   token usage? This changes model-facing payloads and needs eval/snapshot
   review.
